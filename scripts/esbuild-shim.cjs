...
async function formatMessages(messages) {
  return formatMessagesViaShim(messages);
}
...
async function build() {
  throw new Error("miniapp-builder esbuild shim does not implement build().");
}
...
const version = `miniapp-builder-ts-shim:${ts.version}`;
